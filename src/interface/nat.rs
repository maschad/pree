...
impl NatDetector {
    ...
    pub fn method(mut self, method: NatDetectionMethod) -> Self {
        self.method = method;
        self
    }
    ...
    pub fn is_behind_nat(&self) -> Result<bool> {
        match self.method {
            ...
            NatDetectionMethod::Both => {
                ...
                match self.detect_via_stun() {
                    Ok(result) => Ok(result),
                    Err(_) => self.detect_via_socket(),
                }
            }
        }
    }
    ...
    fn detect_via_stun(&self) -> Result<bool> {
        ...
        Err(NetworkError::NotImplemented(
            "STUN detection not implemented yet".to_string(),
        ))
    }
