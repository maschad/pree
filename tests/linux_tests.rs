...
fn test_linux_proc_socket_enumeration() {
    ...
    assert!(
        !sockets.is_empty(),
        "Should find sockets using /proc filesystem"
    );
    ...
    let sockets_with_process: Vec<_> = sockets.iter().filter(|s| s.process_id.is_some()).collect();

    assert!(
        !sockets_with_process.is_empty(),
    ...
    for (pid, name) in &process_map {
        ...
        if std::path::Path::new(&proc_path).exists() {
            ...
            if let Ok(proc_name) = fs::read_to_string(&comm_path) {
                ...
                assert!(
                    name.contains(proc_name)
                        || proc_name.contains(name)
                        || name.starts_with(proc_name)
                        || proc_name.starts_with(name),
                    "Process name mismatch: socket='{}', proc='{}'",
                    name,
                    proc_name
                );
            }
...
fn test_linux_proc_net_parsing() {
    ...
    if let Ok(tcp_content) = fs::read_to_string("/proc/net/tcp") {
        ...
        let our_tcp_sockets: Vec<_> = sockets
            .iter()
            .filter(|s| s.protocol == Protocol::Tcp)
            .collect();

        println!(
            "Found {} TCP sockets in our implementation",
            our_tcp_sockets.len()
        );
    ...
    if let Ok(udp_content) = fs::read_to_string("/proc/net/udp") {
        ...
        let our_udp_sockets: Vec<_> = sockets
            .iter()
            .filter(|s| s.protocol == Protocol::Udp)
            .collect();

        println!(
            "Found {} UDP sockets in our implementation",
            our_udp_sockets.len()
        );
...
fn test_linux_tcp_socket_states() {
    ...
    if listening_socket.is_some() {
        println!(
            "Successfully found listening socket on port {}",
            listening_port
        );
    } else {
...
fn test_linux_performance_vs_netstat() {
    ...
    println!(
        "Our implementation found {} sockets in {:?}",
        our_sockets.len(),
        our_duration
    );
    ...
    if let Ok(output) = ss_result {
        ...
        // Fallback to netstat
        let start = Instant::now();
        let netstat_result = Command::new("netstat").args(["-tuln"]).output();
        ...
        if let Ok(output) = netstat_result {
            ...
            println!(
                "netstat found ~{} lines in {:?}",
                netstat_lines, netstat_duration
            );
        }
    }
...
fn test_linux_proc_filesystem_features() {
    ...
    if std::path::Path::new("/proc/net/tcp6").exists() {
        ...
        let sockets = get_sockets_info().expect("Failed to get socket info");
        let ipv6_sockets: Vec<_> = sockets.iter().filter(|s| s.local_addr.is_ipv6()).collect();
...
fn test_linux_socket_extended_info() {
    ...
    let inet_sockets: Vec<_> = sockets.iter().filter(|s| s.local_addr.is_ipv4()).collect();

    let inet6_sockets: Vec<_> = sockets.iter().filter(|s| s.local_addr.is_ipv6()).collect();
...
fn test_linux_networking_features() {
    ...
    for &port in &system_ports {
        let port_sockets: Vec<_> = sockets
            .iter()
    ...
    println!(
        "Found system sockets on {}/{} common ports",
        found_system_sockets,
        system_ports.len()
    );

    // Test localhost vs external addresses
    let localhost_sockets: Vec<_> = sockets
        .iter()
        .filter(|s| s.local_addr.ip().is_loopback())
        .collect();

    let external_sockets: Vec<_> = sockets
        .iter()
    ...
    assert!(
        !localhost_sockets.is_empty(),
        "Should find localhost sockets"
    );
}
